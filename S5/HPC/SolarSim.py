...
def run_ss(executable_location: Union[str, PathLike], control: Union[str, PathLike],
           summary: Union[str, PathLike], history: Union[str, PathLike],
           solartotals: Union[str, PathLike], lock: Optional[mp.Lock] = None) -> None:
    """run solar sim
    ...
    p = sp.Popen([executable_location, control, summary, history, solartotals], stdout=sp.PIPE)  # run solarsim
    ...
    while p.poll() is None:  # wait for the simulation to complete
        if (np.datetime64('now') - t) > np.timedelta64(1, 'h'):  # 1 hour timeout
            print("SolarSim process timeout. Potential deadlock from subprocess.Popen, consider increasing SolarSim "
                  "update interval")
            p.kill()
            break
...
def const_vel(lock: mp.Lock, t_vel: float, executable_location: Union[str, PathLike],
              control: Union[str, PathLike], summary: Union[str, PathLike],
              history: Union[str, PathLike], solartotals: Union[str, PathLike]) -> None:
...
def vel_sweep(vel_list: List[float],
              executable_location: Union[str, PathLike] = "../SolarSim.X") -> None:
    ...
    for v in vel_list:
        const_vel(lock, v, executable_location, "SolarSim.in", f"Summary_{v}.dat", f"History_{v}.dat",
                  f"SolarTotals_{v}.dat")


def vel_sweep_par(vel_list: List[Union[str, PathLike]],
                  executable_location: Union[str, PathLike] = "../SolarSim.X") -> None:
    ...
    for v in vel_list:
        args.append(
            (lock, v, executable_location, "SolarSim.in", f"Summary_{v}.dat", f"History_{v}.dat",
             f"SolarTotals_{v}.dat"))
...
def read_vel_sweep(vel_list: List[float], path: Union[str, PathLike] = './') -> pd.DataFrame:
...
def file_swap(lock: mp.Lock, filename: Union[str, PathLike], runname: Union[str, PathLike],
              executable_location: Union[str, PathLike], control: Union[str, PathLike],
              summary: Union[str, PathLike], history: Union[str, PathLike],
              solartotals: Union[str, PathLike]) -> None:
    ...
    """
    lock.acquire()  # block until SS readin complete (released in run_ss)
    copyfile(filename, runname)
    print("file swapped")
    run_ss(executable_location, control, summary, history, solartotals, lock)


def file_sweep_par(file_list: List[Union[str, PathLike]], runname: Union[str, PathLike],
                   executable_location: Union[str, PathLike] = "../SolarSim.X") -> None:
    ...
    for f in file_list:
        args.append(
            (lock, f, runname, executable_location, "SolarSim.in", f"Summary_{f}", f"History_{f}", f"SolarTotals_{f}"))
    with mp.Pool(min(mp.cpu_count() - (2 * (mp.cpu_count() <= 16)),
...
def file_sweep(file_list: List[Union[str, PathLike]], runname: Union[str, PathLike],
               executable_location: Union[str, PathLike] = "../SolarSim.X") -> None:
    """sweep files by renaming each of the file in list to runname and run ss. Parallel version available as
    file_sweep_par.
    :param file_list: filename to be swapped in for solarsim
    :param runname: name referenced in control file
    :param executable_location: path to SolarSim to execute
    ...
    lock = mp.Manager().Lock()
    for f in file_list:
        file_swap(lock, f, runname, executable_location, "SolarSim.in", f"Summary_{f}", f"History_{f}",
                  f"SolarTotals_{f}")


def read_file_sweep(file_list: List[Union[str, PathLike]], path: Union[str, PathLike]) -> pd.DataFrame:
    ...
    result = pd.DataFrame(index=range(len(file_list)),
                          columns=['file', 'drivingTime', 'DistCovered', 'SoC', 'AverageVel',
                                   'SoCMax', 'SoCMin'])
...
if __name__ == '__main__':  # pragma: no cover
    ...
    duration = read_vel_sweep(vel, r"W:\DataStorage\OwenFoo\portal_from_hamilton")
