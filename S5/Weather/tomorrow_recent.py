...
def send_request(
    ...
    """Obtain the recent weather from tomorrow.io
    ...
    url = f"https://api.tomorrow.io/v4/weather/history/recent?location={latitude},{longitude}&apikey={api_key}"  # url so pylint: disable = line-too-long
    timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M")

    # Send the API request
    logging.debug(
        f"Sending request to tomorrow.io for lat:{latitude} lon:{longitude}")
    try:
