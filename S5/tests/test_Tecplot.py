...
def test_tecplotheaderzone_constructor():
    ...
    assert isinstance(TestTPHeaderZone.zonetitle, str)
    assert isinstance(TestTPHeaderZone.ni, int)
    assert isinstance(TestTPHeaderZone.nj, int)
    assert isinstance(TestTPHeaderZone.nk, int)
    assert isinstance(TestTPHeaderZone.F, str)
...
def test_read_bad_title(tmp_path):
    ...
    with open(filepath, "w") as file:
        file.write(
            """"Velocity file generated by S5.HPC"
...
def test_read_bad_var_title(tmp_path):
    ...
    with open(filepath, "w") as file:
        file.write(
            """Title = "Velocity file generated by S5.HPC"
...
def test_history_add_timestamp(history_file, count):
    ...
    hist.data["DDHHMMSS"] = [
        int(str((entry.day - baseline.day + 1)) + entry.strftime("%H%M%S"))
        for entry in correct_datetime
    ]
...
def test_weather_add_timestamp(weather_file, count):
    ...
    weather.data["Day"] = [
        int(str((entry.day - baseline.day + 1))) for entry in correct_datetime
    ]
    weather.data["Time(HHMM)"] = [
        int(str(entry.strftime("%H%M"))) for entry in correct_datetime
    ]
...
def test_weather_add_day_time_cols_invalid_index(weather_file):
    weather = TP.SSWeather(weather_file)
    with pytest.raises(
            TypeError, match="Data index should be pd.DateTimeIndex."
    ):
        weather.add_day_time_cols()
...
def test_DSWinput_get_value(solarsim_in):
    ...
    assert (
            ssin.get_value("Title").strip()
            == r'"SolarSim4.1, WSC, Aero202, CRR(Schwalbe)=0.013, 2xDriveTek MPPTs, '
...
@pytest.mark.parametrize(
    "original,converted",
    [("win", "lin"), ("lin", "win"), ("win", "win"), ("lin", "lin")],
)
